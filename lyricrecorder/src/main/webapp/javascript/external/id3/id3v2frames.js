...
var pictureType = [ "32x32 pixels 'file icon' (PNG only)", "Other file icon",
...
function getTime(duration) {
	var duration = duration / 1000, seconds = Math.floor(duration) % 60, minutes = Math
			.floor(duration / 60) % 60, hours = Math.floor(duration / 3600);
...
function formatTime(time) {
	...
	var minutes = (time.hours > 0 && time.minutes < 10) ? '0' + time.minutes
			: time.minutes;

	return (time.hours > 0 ? time.hours + ':' : '') + minutes + ':' + seconds;
}

ID3v2.readFrameData['APIC'] = function readPictureFrame(offset, length, data,
		flags, v) {
	v = v || '3';
	...
	case '3':
	case '4':
		var format = data.getStringWithCharsetAt(offset + 1, length
				- (offset - start), '');
	...
	var desc = data.getStringWithCharsetAt(offset + 1, length
			- (offset - start), charset);
...
ID3v2.readFrameData['COMM'] = function readCommentsFrame(offset, length, data) {
	...
	var shortdesc = data
			.getStringWithCharsetAt(offset + 4, length - 4, charset);

	offset += 4 + shortdesc.bytesReadCount;
	var text = data.getStringWithCharsetAt(offset, (start + length) - offset,
			charset);
...
ID3v2.readFrameData['TXXX'] = function readTextFrame(offset, length, data) {
	var charset = getTextEncoding(data.getByteAt(offset));
	var description = data.getStringWithCharsetAt(offset + 1, length - 1,
			charset).toString();
	var start = offset;
	var language = data.getStringAt(offset + 1, 3);
	var shortdesc = data
...
ID3v2.readFrameData['T*'] = function readTextFrame(offset, length, data) {
	var charset = getTextEncoding(data.getByteAt(offset));
	return data.getStringWithCharsetAt(offset + 1, length - 1, charset)
			.toString();
};
...
ID3v2.readFrameData['USLT'] = function readLyricsFrame(offset, length, data) {
	...
	var charset = getTextEncoding(data.getByteAt(offset));
	var language = data.getStringAt(offset + 1, 3);
	var descriptor = data.getStringWithCharsetAt(offset + 4, length - 4,
			charset);

	offset += 4 + descriptor.bytesReadCount;
	var lyrics = data.getStringWithCharsetAt(offset, (start + length) - offset,
			charset);
